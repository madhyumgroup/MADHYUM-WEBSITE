...
(function () {
  ...
  const S = function (terms, page, label) {
    return terms.split('|').map(function (term) {
  ...
  function renderSearch(query) {
    ...
    if (!raw) {
      searchResults.innerHTML =
        'Start typing a requirement. Try property, Dubai, honeymoon, MBBS, GST, wedding, Hajj, or membership.';
      return;
    }
    ...
    const matches = SEARCH_DATA
      .map(function (item) {
        ...
        const haystack = termText.toLowerCase();
      ...
      .sort(function (a, b) {
        return b[3] - a[3] || a[0].length - b[0].length;
      })
      .filter(function (item) {
        const term = item[0];
        const page = item[1];
    ...
    if (!matches.length) {
      searchResults.innerHTML =
        'No matching keyword found. Try a broader requirement or service name.';
      return;
    }
    ...
    searchResults.innerHTML = Object.entries(pages)
      .map(function (entry) {
        const page = entry[0];
        const value = entry[1];
        ...
        return (
          ...
          '</strong>' +
          '<small>' +
          esc(value.terms.slice(0, 8).join(' • ')) +
          '</small>' +
          '</a>'
        );
      })
  ...
  document
    ...
    .forEach(function (image) {
      image.addEventListener('error', function () {
        ...
        if (file && !image.dataset.fallback) {
  ...
  const prev = document.querySelector('#layeredPrev');
  const next = document.querySelector('#layeredNext');
  const dots = document.querySelector('#layeredDots');
  ...
  if (wings.length) {
    if (dots) {
      ...
      wings.forEach(function (wing, index) {
        ...
        dot.setAttribute(
          'aria-label',
          'Show ' +
            (wing.getAttribute('aria-label') || 'business')
        );
    ...
    function update() {
      ...
      wings.forEach(function (wing, index) {
        ...
        const className = classes[relative];
    ...
    const hero = document.querySelector('.layered-hero');
  ...
  /* =========================================================
     CORS-FREE INQUIRY SUBMISSION
  ========================================================= */

  function submitInquiryWithoutCors(
    payload
  ) {
    try {
      if (
